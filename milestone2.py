...
def classify_usb(contour, image):
    ...
    width_ratio = abs((rightmost[0]-leftmost[0]) / (bottommost[0]-topmost[0]) if (bottommost[0]-topmost[0]) != 0 else 1)

    # Strict classification rules
    if 2.2 <= aspect_ratio <= 4.0 and 1.0 <= width_ratio <= 6:
        return f"USB-C (Oval)(AR:{aspect_ratio:.1f}, WR:{width_ratio:.1f}, ER:{ellipse_ratio:.1f})"
    elif 2.1 <= aspect_ratio <= 2.8 :
        return f"USB-A (Rectangle)(AR:{aspect_ratio:.1f}, WR:{width_ratio:.1f}, ER:{ellipse_ratio:.1f})"
    elif 1.0 <= aspect_ratio <= 1.2 and solidity > 0.95:
        return f"USB-B (Square) (AR:{aspect_ratio:.1f}, WR:{width_ratio:.1f}, ER:{ellipse_ratio:.1f})"
    elif 1.5 <= aspect_ratio <= 2.0:
        return f"Mini-USB (Beveled) (AR:{aspect_ratio:.1f}, WR:{width_ratio:.1f}, ER:{ellipse_ratio:.1f})"
    elif 1.3 <= aspect_ratio <= 2.5 and width_ratio > 4.2:
        return f"Micro-USB (Trapezoid) (AR:{aspect_ratio:.1f}, WR:{width_ratio:.1f}, ER:{ellipse_ratio:.1f})"
    else:
